  ...
  const formatReleaseDate = (dateString) => {
    ...
    return ( 
        <main className="w-100%  md:flex-row flex flex-col">
            <div className={`absolute md:relative md:block bg-white h-screen rounded-tr-10 rounded-br-10 border border-gray-600 border-solid z-1 w-50% md:w-16% mr-5 ${activeClass}`}  >
          ...
          {loading ? (
            <p className="text-xl text-center animate-ping duration-500">Loading...</p>
              ...
              // Display movie details once loading is complete
                <div className="w-full relative px-4 md:py-none">
                    <div className="w-full relative ">
                        ...
                        alt={movie?.title}
                        loading="lazy" 
                        className="w-full h-100 md:h-full rounded-4"/>
              ...
              <div className="absolute  top-0 right-0 w-full h-60 md:h-[300px] lg:h-[449px] rounded-xl overflow-hidden flex justify-center  items-center  ">
                    <i className="fa fa-play hover-text-#B91C1C text-white motion-safe:animate-pulse  duration-500 cursor-pointer  text-7xl"></i>
                </div>
                    </div>
